...
public class DropsSetCurrentHourCommand extends CommandAPICommand {
	...
	public DropsSetCurrentHourCommand(String commandName) {
		...
		executesPlayer((player, args) -> {
			...
			if (hour < 0) {
				Chat.sendMessage(player, Component.text("Die Stunde muss >= 0 sein.", NamedTextColor.RED));
			...
			Chat.sendMessage(player, Component.text("Die Stunde wurde auf ", NamedTextColor.GREEN)
